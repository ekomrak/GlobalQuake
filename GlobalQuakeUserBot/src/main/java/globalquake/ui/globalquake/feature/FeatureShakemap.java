...
public class FeatureShakemap extends RenderFeature<IntensityHex> {
    ...
    public FeatureShakemap() {
        ...
        GlobalQuakeClient.instance.getLocalEventHandler().registerEventListener(new GlobalQuakeLocalEventListener(){
            @Override
    ...
    private void updateHexes() {
        Map<Long, IntensityHex> items = new HashMap<>();
        for(var pair : GlobalQuakeClient.instance.getShakemapService().getShakeMaps().entrySet().stream()
