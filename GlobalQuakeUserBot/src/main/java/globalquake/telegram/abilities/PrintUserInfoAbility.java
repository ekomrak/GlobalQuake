                    ...
                    try {
                        ...
                        if (telegramUser != null) {
                            ...
                            getTelegramService().getSilent().sendMd(("*Пользователь*%nId: %d%nChatId: %d%nFirstName: %s%nLastName: %s%nUserName: %s%nLanguageCode: %s%nPremium: %s%nEnabled: %s%nSubscriptionDate: %s%nUpdatedDate: %s%n%n".formatted(telegramUser.getId(), telegramUser.getChatId(), telegramUser.getFirstName(), telegramUser.getLastName(), telegramUser.getUserName(), telegramUser.getLanguageCode(), TelegramUtils.booleanToString(telegramUser.getPremium()), TelegramUtils.booleanToString(telegramUser.getEnabled()), telegramUser.getSubscriptionDate().format(TelegramUtils.DATE_FORMAT), telegramUser.getUpdatedDate().format(TelegramUtils.DATE_FORMAT)) +
                                    ...
                                    "*Высокий уровень датчиков*%nПолучать уведомления: %s%nПолучать геолокацию: %s%nПолучать картинку: %s%nПолучать карту: %s%nРадиус: %d км.%nИнтенсивность: %d%n%n".formatted(TelegramUtils.booleanToString(telegramUser.getEnableTelegramStationHighIntensityAlert()), TelegramUtils.booleanToString(telegramUser.getEnableTelegramStationHighIntensityLocation()), TelegramUtils.booleanToString(telegramUser.getEnableTelegramStationHighIntensityImage()), TelegramUtils.booleanToString(telegramUser.getEnableTelegramStationHighIntensityMap()), telegramUser.getTsStationMaxDist(), telegramUser.getTsStationMinIntensity()) +
