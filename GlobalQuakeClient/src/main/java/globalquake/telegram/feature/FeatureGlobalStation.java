...
public class FeatureGlobalStation extends RenderFeature<AbstractStation> {
    ...
    public void createPolygon(GlobeRenderer renderer, RenderEntity<AbstractStation> entity, RenderProperties renderProperties) {
        ...
        switch (inputType) {
            case UNKNOWN -> renderer.createCircle(elementStationCircle.getPolygon(),
                    entity.getOriginal().getLatitude(),
                    entity.getOriginal().getLongitude(),
                    size, 0, 30);
            case VELOCITY -> renderer.createTriangle(elementStationCircle.getPolygon(),
                    entity.getOriginal().getLatitude(),
                    entity.getOriginal().getLongitude(),
                    size * 1.41, 0, 0);
            case ACCELERATION -> renderer.createTriangle(elementStationCircle.getPolygon(),
                    entity.getOriginal().getLatitude(),
                    entity.getOriginal().getLongitude(),
                    size * 1.41, 0, 180);
            case DISPLACEMENT -> renderer.createSquare(elementStationCircle.getPolygon(),
                    entity.getOriginal().getLatitude(),
                    entity.getOriginal().getLongitude(),
                    size * 1.41, 0);
        }
    ...
    public void render(GlobeRenderer renderer, Graphics2D graphics, RenderEntity<AbstractStation> entity, RenderProperties renderProperties) {
        ...
        } else if (entity.getOriginal().isInEventMode()) {
            Color c = Color.green;
        ...
        graphics.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_OFF);
        drawDetails(renderProperties.scroll, centerPoint, graphics, entity.getOriginal(), renderer, entity, renderProperties);
    }

    private void drawDetails(double scroll, Point2D centerPoint, Graphics2D g, AbstractStation station, GlobeRenderer renderer,
