    ...
    public Ability mySettings() {
        return Ability.builder()
                ...
                .action(ctx -> getTelegramService().getSilent().sendMd(("*Координаты дома*%nШирота: %.6f%nДолгота: %.6f%n%n" +
                        ...
                        "*Высокий уровень датчиков*%nПолучать уведомления: %s%nПолучать геолокацию: %s%nПолучать картинку: %s%nПолучать карту: %s%nЗона 1:%nРадиус: %.1f км.%nИнтенсивность: %.1f%nЗона 2:%nРадиус: %.1f км.%nИнтенсивность: %.1f").formatted(Settings.homeLat, Settings.homeLon,
                        ...
                        TelegramUtils.booleanToString(Settings.enableTelegramStationHighIntensityAlert), TelegramUtils.booleanToString(Settings.enableTelegramStationHighIntensityLocation), TelegramUtils.booleanToString(Settings.enableTelegramStationHighIntensityImage), TelegramUtils.booleanToString(Settings.enableTelegramStationHighIntensityMap), Settings.tsStationMaxDist1, Settings.tsStationMinIntensity1, Settings.tsStationMaxDist2, Settings.tsStationMinIntensity2), ctx.chatId()))
                .build();
    }
}
